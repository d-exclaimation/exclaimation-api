...
type Post struct {
	...
	// Crabrave holds the value of the "crabrave" field.
	// The amount of positive feedback
	Crabrave int `json:"crabrave,omitempty"`
}
...
func (*Post) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		switch columns[i] {
		case post.FieldID, post.FieldCrabrave:
	...
	for i := range columns {
		...
		case post.FieldCrabrave:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field crabrave", values[i])
			} else if value.Valid {
				po.Crabrave = int(value.Int64)
			}
		}
	}
	return nil
...
func (po *Post) String() string {
	...
	builder.WriteString(", crabrave=")
	builder.WriteString(fmt.Sprintf("%v", po.Crabrave))
	builder.WriteByte(')')
